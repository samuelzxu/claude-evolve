...
Ported from ShinkaEvolve/shinka/prompts/prompts_cross.py.
"""
...
def get_cross_component(
    ...
    """Sample a random inspiration and format it as a crossover context block."""
    if not inspirations:
        return ""

    inspiration = random.choice(inspirations)

    crossover_str = "# Crossover Inspiration Programs\n"
    crossover_str += f"```{language}\n{inspiration.code}\n```\n\n"
    crossover_str += f"Performance metrics: {perf_str(inspiration.combined_score)}\n\n"

    return crossover_str
